...
import (
	...
	"github.com/ethereumai/go-ethereumai/accounts/usbwallet/internal/trezor"
...
// or whether the EthereumAI app was not started on it.
...
// EthereumAI address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.EthereumAIAddress)
	if _, err := w.trezorExchange(&trezor.EthereumAIGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.EthereumAISignTx{
	...
	response := new(trezor.EthereumAITxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.EthereumAITxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}

...
import (
	...
	"github.com/ethereumai/go-ethereumai/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Geai console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = eai.getBlock(eai.getTransaction(tx).blockHash);
  var genesis = eai.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.eai.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  eai.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(eaidb.NewMemDatabase(), test.Genesis.Alloc)

...
import (
	...
	geaimetrics "github.com/ethereumai/go-ethereumai/metrics"
	"github.com/ethereumai/go-ethereumai/metrics/influxdb"
...
func Setup(ctx *cli.Context) {
	if geaimetrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(geaimetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}

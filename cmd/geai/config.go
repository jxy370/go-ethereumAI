...
type eaistatsConfig struct {
	URL string `toml:",omitempty"`
}

type geaiConfig struct {
	Eai       eai.Config
	Shh       whisper.Config
	Node      node.Config
	Eaistats  eaistatsConfig
	Dashboard dashboard.Config
}

func loadConfig(file string, cfg *geaiConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "eai", "shh")
	cfg.WSModules = append(cfg.WSModules, "eai", "shh")
	cfg.IPCPath = "geai.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, geaiConfig) {
	// Load defaults.
	cfg := geaiConfig{
		Eai:       eai.DefaultConfig,
	...
	utils.SetEaiConfig(ctx, stack, &cfg.Eai)
	if ctx.GlobalIsSet(utils.EaiStatsURLFlag.Name) {
		cfg.Eaistats.URL = ctx.GlobalString(utils.EaiStatsURLFlag.Name)
	}
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	// Add the EthereumAI Stats daemon if requested.
	if cfg.Eaistats.URL != "" {

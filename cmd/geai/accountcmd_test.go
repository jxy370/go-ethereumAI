...
func TestAccountListEmpty(t *testing.T) {
	geai := runGeai(t, "account", "list")
	geai.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	geai := runGeai(t, "account", "list", "--datadir", datadir)
	defer geai.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	geai := runGeai(t, "account", "new", "--lightkdf")
	defer geai.ExpectExit()
	geai.Expect(`
...
`)
	geai.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	geai := runGeai(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer geai.ExpectExit()
	geai.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(geai.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(geai.StderrText(), m) {
...
// https://github.com/ethereumai/go-ethereumai/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	geai.SetTemplateFunc("keypath", func(file string) string {

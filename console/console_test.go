...
func newTester(t *testing.T, confOverride func(*eai.Config)) *tester {
	...
	// Create a networkless protocol stack and start an EthereumAI service within
	...
	eaiConf := &eai.Config{
		...
		EtherAIbase: common.HexToAddress(testAddress),
		Eaiash: eaiash.Config{
			PowMode: eaiash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(eaiConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return eai.New(ctx, eaiConf) }); err != nil {
		t.Fatalf("failed to register EthereumAI protocol: %v", err)
	}
	...
	stack.Service(&ethereumai)

	return &tester{
		workspace: workspace,
		stack:     stack,
		ethereumai:  ethereumai,
		console:   console,
		input:     prompter,
		output:    printer,
	}

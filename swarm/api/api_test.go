...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.eai"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.eai"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.eai"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	eaiAddr := "swarm.eai"
	eaiHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	eaiResolve := newTestResolver(eaiHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(eaiResolve, "")),
			addr:   eaiAddr,
			result: eaiHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(eaiResolve, ""),
				MultiResolverOptionWithResolver(eaiResolve, ""),
			),
			addr:   eaiAddr,
			result: eaiHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(eaiResolve, "eai"),
			),
			addr:   eaiAddr,
			result: eaiHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "eai"),
				MultiResolverOptionWithResolver(doesntResolve, "eai"),
				MultiResolverOptionWithResolver(eaiResolve, "eai"),
			),
			addr:   eaiAddr,
		...
		},
		{
			...
			addr:   eaiAddr,
			result: eaiHash,
			err:    errors.New(`DNS name not found: "swarm.eai"`),
		},
		{

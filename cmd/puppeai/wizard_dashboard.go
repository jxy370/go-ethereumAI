...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"eaistats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "eaistats":
				if infos, err := checkEaistats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have eaistats running, ask whether to make the secret public or not
	if w.conf.eaistats != "" {
		fmt.Println()
		fmt.Println("Include eaistats secret on dashboard (y/n)? (default = yes)")

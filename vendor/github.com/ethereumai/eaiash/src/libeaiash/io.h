...
// Reference: https://github.com/ethereumai/wiki/wiki/Eaiash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see eaiash_io_prepare
enum eaiash_io_rc {
	EAIASH_IO_FAIL = 0,           ///< There has been an IO failure
	EAIASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	EAIASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	EAIASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of eaiash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * eaiash_full_t
 */
#ifdef EAIASH_PRINT_CRITICAL_OUTPUT
#define EAIASH_CRITICAL(...)							\
	do													\
	{													\
		printf("EAIASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for eaiash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the eaiash
 ...
 *                           https://github.com/ethereumai/wiki/wiki/Eaiash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum eaiash_io_rc
 */
 ...
 *
 * The spec defining this directory is here: https://github.com/ethereumai/wiki/wiki/Eaiash-DAG
 *
...
static inline bool eaiash_io_mutable_name(
	uint32_t revision,

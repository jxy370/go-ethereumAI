...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for eaistats availability")
	if infos, err := checkEaistats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["eaistats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["eaistats"] = infos.Report()
		eaistats = infos.config
	}
